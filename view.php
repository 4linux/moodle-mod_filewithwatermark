...
if ($displaytype == fileutil::$DISPLAY_OPEN || $displaytype == fileutil::$DISPLAY_DOWNLOAD) {
    $redirect = true;
}
...
if ($redirect && !$forceview) {
    ...
    $fullurl = moodle_url::make_file_url('/pluginfile.php', $path, $displaytype == fileutil::$DISPLAY_DOWNLOAD);

    redirect($fullurl);
}

switch ($displaytype) {
    case fileutil::$DISPLAY_EMBED:
        filewithwatermark_display_embed($filewithwatermark, $cm, $course, $file);
        break;
    case fileutil::$DISPLAY_FRAME:
